...
parser.add_argument("-ga", "-gamma_area", "--gamma_area", type=float, default=0.0)
parser.add_argument(
...
if args.data.lower() == "mnist":
    ...
    TEST_DATA_FILE = "./data/multi_mnist_data/test{}.tfrecords".format(
        name_of_common_test)
    CANVAS_SIZE = 50
    if "bbox" in args.dig_surfix:
        size_min, size_max = 11, 15
    else:
        size_min, size_max = 17, 23
    ...
    CANVAS_SIZE = 64
    if "bbox" in args.dig_surfix:
        size_min, size_max = 12, 15
    else:
        size_min, size_max = 20, 25
...
for i in range(2):
    ...
    models.append(
        AIRModel(
            ...
            constrains_area_gamma=args.gamma_area,
            constrains_area_minmax=[size_min, size_max],
            fix_steps=NUM_OF_DIGITS_TRAIN[0] if len(NUM_OF_DIGITS_TRAIN) == 1 else None,
            annealing_schedules={
                # "constrains_area_gamma": {
                #     "init": 1e-10,
                #     "min": 0.0,
                #     "max": args.gamma_bbox,
                #     "factor": 10e11,
...
with tf.Session(config=session_conf) as sess:
    ...
    returned_image = np.reshape(returned_image, [-1, CANVAS_SIZE, CANVAS_SIZE, 1])
    tmp_image = np.ones([returned_image.shape[0], CANVAS_SIZE + 2, CANVAS_SIZE + 2, 1])
    tmp_image[:, :CANVAS_SIZE, :CANVAS_SIZE, :] = returned_image
    pile_image(
        tmp_image,
    ...
    log.info("Training...\n")
    min_loss, best_elbo, best_accu, best_mse, best_iou = 9999999, 0., 0., 0., 0.
    update_flag = False
        ...
        while True:
            ...
            if step % LOG_EACH_ITERATION == 0:
                ...
                for name, value in zip(train_model.log_variables_name, logged_results):
                    if name == "TotLoss" and np.all(np.mean(value) < min_loss):
                        min_loss = np.mean(value)
                        update_flag = True
            ...
            if step % TESET_EACH_ITERATION == 0:
                ...
                test_for_best = {
                    a: b
                    for a, b in zip(test_model.log_variables_name, logged_results_test)
                }
                if update_flag is True:
                    update_flag = False
                    best_elbo = np.mean(test_for_best['elbo'])
                    best_accu = np.mean(test_for_best['accu'])
                    best_mse = np.mean(test_for_best['mse'])
                    best_iou = global_iou
