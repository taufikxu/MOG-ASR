...
class AIRModel:
    ...
    def __init__(self,
                 ...
                 constrains_area_gamma=0.,
                 constrains_area_minmax=[0., 0.],
                 fix_steps=None):
        ...
        if fix_scale_distribution is True:
            self.scale_prior_mean = -1.0  # if canvas_size < 51 else -1.5
        ...
        self.constrains_area_gamma = constrains_area_gamma
        self.constrains_area_minmax = constrains_area_minmax
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            if annealing_schedules is not None:
                for param, schedule in annealing_schedules.items():
                    ...
                    value = self._create_annealed_tensor(param, schedule,
                                                         self.global_step)
                    self.log_variables.update({param: value})
    ...
    def _create_model(self):
        ...
        def body(
            ...
            with tf.variable_scope("inf_scale"):
                ...
                scale_mean = tf.layers.dense(
                    tf.concat([hidden_m, shift_latent], axis=-1), 1, activation=None)
                ...
                scale_log_variance = tf.layers.dense(
                    tf.concat([hidden_v, shift_latent], axis=-1), 1, activation=None)
                ...
                theta_recon = tf.stack(
                    [
                        tf.concat(
                            [
                                tf.stack(
                                    [1.0 / inf_s, tf.zeros_like(inf_s)], axis=1),
                                tf.expand_dims(-inf_x / inf_s, 1),
                            ],
                            axis=1,
                        ),
                        tf.concat(
                            [
                                tf.stack([tf.zeros_like(inf_s), 1.0 / inf_s], axis=1),
                                tf.expand_dims(-inf_y / inf_s, 1),
                            ],
                            axis=1,
                        ),
                    ],
            ...
            with tf.variable_scope("z_pres"):
                with tf.variable_scope("prior"):
                    if self.fix_steps is not None:
                        z_pres_prior_log_odds = tf.where(
                            tf.less(step,
                                    self.fix_steps), 100 * tf.ones([self.batch_size]),
                            -100 * tf.ones([self.batch_size]))
                    else:
        ...
        with tf.variable_scope("air_model"):
            ...
            _, _, _, _, _, _, _, reconstruction, loss_kl_dict, loss_pr_dict, self.rec_num_digits, scales, shifts, z_pres_probs, st_backward, windows, latents = tf.while_loop(
                ...
                [
                    ...
                    tf.zeros([self.batch_size, 3]),  # TODO: previous shift and scale
                    tf.zeros_like(
        ...
        with tf.variable_scope("loss/pr_num_margin"):
            ...
            if self.constrains_margin_gamma > 1e-8:
                ...
                # labels = 200 * objective[:, :, :steps_length] - 100
                # constrains_num_element_min = -1 * z_pres_rep_prob * labels + tf.maximum(
        ...
        with tf.variable_scope("loss/pr_size"):
            scale = self.rec_scales * self.canvas_size
            max_penalty = tf.maximum(self.constrains_area_minmax[1] - scale,
                                     tf.zeros_like(scale))
            # max_penalty = tf.square(max_penalty)
            min_penalty = tf.maximum(scale - self.constrains_area_minmax[0],
                                     tf.zeros_like(scale))
            # min_penalty = tf.square(min_penalty)
            self.area_loss = tf.reduce_mean(
                tf.layers.flatten(max_penalty + min_penalty), -1)
            self.log_variables.update({"area_loss": self.area_loss})
            pr_loss += self.constrains_area_gamma * self.area_loss
        ...
        with tf.variable_scope("loss/pr_bbox"):
            ...
            scale = self.rec_scales * csize

            min_x = center_x - 0.5 * scale
            min_y = center_y - 0.5 * scale
            max_x = center_x + 0.5 * scale
            max_y = center_y + 0.5 * scale
            out_loss = tf.maximum(-1 * min_x, tf.zeros_like(min_x)) + tf.maximum(
                -1 * min_y, tf.zeros_like(min_y)) + tf.maximum(
                    max_x - csize, tf.zeros_like(max_x)) + tf.maximum(
                        max_y - csize, tf.zeros_like(max_y))
            self.out_loss = tf.reduce_sum(tf.layers.flatten(out_loss), -1)
            self.log_variables.update({"out_loss": self.out_loss})

            size_loss = tf.abs(self.rec_scales * csize -
                               tf.transpose(self.rec_scales * csize, [0, 2, 1]))
            size_loss = tf.maximum(size_loss - 3, tf.zeros_like(size_loss))
            ...
            max_diff = tf.maximum(x_diff, y_diff)
            scale_mean = (scale + tf.transpose(scale, [0, 2, 1])) / 2.
            overlap_loss = tf.maximum(scale_mean - max_diff, tf.zeros_like(scale_mean))
            ...
            if self.constrains_bbox_gamma is not None:
                ...
                pr_loss += self.out_loss * self.constrains_bbox_gamma
    ...
    def _create_generation(self):
        ...
        def body(
            ...
            gen_scale_latent = self._sample_from_mvn(
                gen_scale_mean, gen_scale_variance, shape=[batch_size, 1])
            ...
            with tf.variable_scope("z_pres"):
                with tf.variable_scope("prior"):
                    if self.fix_steps is not None:
                        z_pres_prior_log_odds = tf.where(
                            tf.less(step, self.fix_steps),
                            100 * tf.ones([self.generation_batch_size]),
                            -100 * tf.ones([self.generation_batch_size]))
                    else:
                        hidden = tf.layers.dense(
                            gen_prev_output,
        ...
        with tf.variable_scope("air_model"):
            ...
            # RNN while_loop with variable number of steps for each batch item
            _, _, _, _, _, _, reconstruction, rec_num_digits, rec_st_back = tf.while_loop(
                cond,
                body,
                [
                    tf.constant(0),  # RNN time step, initially zero
