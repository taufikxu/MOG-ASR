...
class AIRModel:
    ...
    def __init__(
        ...
        if num_prior is not None:
            objective = np.zeros([self.max_steps])
            buffer = 1. - 1. / len(self.num_prior)
            for ind in range(self.max_steps):
                if ind not in self.num_prior and ind < max(self.num_prior):
                    objective[ind] = 1.
                else:
                    if ind == max(self.num_prior):
                        break
                    objective[ind] = buffer
                    if ind in self.num_prior:
                        buffer -= 1. / len(self.num_prior)

            for ind in range(self.max_steps):
                if objective[ind] == 1.:
                    objective[ind] = 100
                elif objective[ind] == 0.:
                    objective[ind] = -100.
                else:
                    objective[ind] = np.log(objective[ind] / (1. - objective[ind]))

            self.marginal_objective = tf.convert_to_tensor(objective, dtype=tf.float32)
        ...
        with tf.variable_scope(scope, reuse=True):
            ...
            self.generated_samples, self.generated_samples_bbox = self._create_generation(
            )
    ...
    def _draw_colored_bounding_boxes(images, boxes, steps, max_steps=3):
        ...
        for s in range(max_steps):
            ...
            for c in range(3):
                channels_t = tf.where(
                    tf.greater(step_box, 0),
                    tf.ones_like(channels[c]) * colors[s][c],
                    channels[c],
                )
                channels[c] = tf.where(tf.less(s, steps), channels_t, channels[c])
    ...
    def _visualize_generations(self, original, st_back, steps, zoom):
        # enlarging the original images
        large_original = tf.image.resize_images(
            tf.reshape(original, [-1, self.canvas_size, self.canvas_size, 1]),
            [zoom * self.canvas_size, zoom * self.canvas_size],
    ...
    def _create_model(self):
        ...
        def body(
                ...
                with tf.variable_scope("mean"):
                    with tf.variable_scope("hidden") as scope:
                        hidden = layers.fully_connected(
                            outputs, self.scale_hidden_units, scope=scope)
                    with tf.variable_scope("output") as scope:
                        scale_mean = layers.fully_connected(
                            hidden, 1, activation_fn=None, scope=scope)
                with tf.variable_scope("log_variance"):
                    with tf.variable_scope("hidden") as scope:
                        hidden = layers.fully_connected(
                            outputs, self.scale_hidden_units, scope=scope)
                    with tf.variable_scope("output") as scope:
                        scale_log_variance = layers.fully_connected(
                            hidden, 1, activation_fn=None, scope=scope)
            ...
            with tf.variable_scope("shift"):
                ...
                with tf.variable_scope("mean"):
                    with tf.variable_scope("hidden") as scope:
                        hidden = layers.fully_connected(
                            outputs, self.shift_hidden_units, scope=scope)
                    with tf.variable_scope("output") as scope:
                        shift_mean = layers.fully_connected(
                            hidden, 2, activation_fn=None, scope=scope)
                with tf.variable_scope("log_variance"):
                    with tf.variable_scope("hidden") as scope:
                        hidden = layers.fully_connected(
                            outputs, self.shift_hidden_units, scope=scope)
                    with tf.variable_scope("output") as scope:
                        shift_log_variance = layers.fully_connected(
                            hidden, 2, activation_fn=None, scope=scope)
            ...
            with tf.variable_scope("z_pres"):
                ...
                with tf.variable_scope("log_odds"):
                    with tf.variable_scope("hidden") as scope:
                        hidden = layers.fully_connected(
                            outputs, self.z_pres_hidden_units, scope=scope)
                    with tf.variable_scope("output") as scope:
                        z_pres_log_odds = layers.fully_connected(
                            hidden, 1, activation_fn=None, scope=scope)[:, 0]
            ...
            with tf.variable_scope("loss/z_pres_kl"):
                ...
                if self.num_prior is not None:
                    bias = self.marginal_objective[step]
                    z_pres_kl_end = concrete_binary_kl_mc_sample(
                        z_pres_pre_sigmoid, -100, self.z_pres_temperature,
                        z_pres_log_odds, self.z_pres_temperature)
                else:
                    bias = 0.0
                    z_pres_kl_end = tf.zeros_like(z_pres_log_odds)

                z_pres_kl = concrete_binary_kl_mc_sample(
                    z_pres_pre_sigmoid,
                    self.z_pres_prior_log_odds + bias,
            ...
            with tf.variable_scope("loss/scale_kl"):
                ...
                scale_kl = 0.5 * tf.reduce_sum(
                    self.scale_prior_log_variance - scale_log_variance - 1.0 +
                    scale_variance / self.scale_prior_variance + tf.square(
                        scale_mean - self.scale_prior_mean) / self.scale_prior_variance,
                    1,
                )
            ...
            with tf.variable_scope("loss/shift_kl"):
                ...
                shift_kl = 0.5 * tf.reduce_sum(
                    self.shift_prior_log_variance - shift_log_variance - 1.0 +
        ...
        with tf.variable_scope("rnn") as rnn_scope:
            ...
            _, _, _, reconstruction, loss, self.rec_num_digits, scales, shifts, z_pres_probs, z_pres_kls, scale_kls, shift_kls, vae_kls, st_backward, windows, latents = tf.while_loop(
                cond,
                body,
                [
                    ...
                    tf.TensorArray(dtype=tf.float32, size=0,
                                   dynamic_size=True),  # z_pres KL-divergence
                    tf.TensorArray(dtype=tf.float32, size=0,
                                   dynamic_size=True),  # scale KL-divergence
                    tf.TensorArray(dtype=tf.float32, size=0,
                                   dynamic_size=True),  # shift KL-divergence
                    tf.TensorArray(dtype=tf.float32, size=0,
                                   dynamic_size=True),  # VAE KL-divergence
                    tf.TensorArray(dtype=tf.float32, size=0,
        ...
        with tf.variable_scope("loss/reconstruction"):
            ...
            self.reconstruction_loss = -tf.reduce_sum(
                self.input_images * tf.log(self.reconstruction + epsilon) +
                (1.0 - self.input_images) * tf.log(1.0 - self.reconstruction + epsilon),
                1,
                name="reconstruction_loss",
            )
            self.mse_loss = tf.reduce_sum(
                tf.square(self.input_images - self.reconstruction), 1)
        ...
        with tf.variable_scope("summaries"):
            ...
            self.mse_loss = tf.reduce_mean(self.mse_loss, name='mse')

            # post while-loop numeric summaries grouped by digit count
            self._summarize_by_digit_count(self.rec_num_digits, self.target_num_digits,
                                           "steps")
            self._summarize_by_digit_count(self.reconstruction_loss,
                                           self.target_num_digits, "rec_loss")
            self._summarize_by_digit_count(accuracy, self.target_num_digits,
                                           "digit_acc")
            ...
            self._summarize_by_step(self.rec_scales[:, :, 0], self.rec_num_digits,
                                    "scale")
            ...
            with tf.variable_scope("training"):
                ...
                if self.gradient_clipping_norm is not None:
                    for i in range(len(grads)):
                        ...
                        #     tf.summary.histogram(variables[i].name + "_grad_original",
                        #                          grads[i]))
                        # self.grad_summaries.append(
                        #     tf.summary.scalar(
                ...
                for i in range(len(grads)):
                    ...
                    self.grad_summaries.append(
                        tf.summary.histogram(variables[i].name + "_grad_applied",
                                             grads[i]))
                    self.grad_summaries.append(
                        tf.summary.scalar(variables[i].name + "_grad_applied_norm",
                                          tf.norm(grads[i])))
                    self.grad_summaries.append(
                        tf.summary.scalar(
    ...
    def _create_generation(self):
        ...
        # condition of tf.while_loop
        def cond(step, stopping_sum, *_):
            return tf.less(step, self.max_steps_generation_placeholder)
            # return tf.logical_and(
            #     tf.less(step, self.max_steps),
            #     tf.reduce_any(tf.less(stopping_sum, self.stopping_threshold)),
            # )
        ...
        # body of tf.while_loop
        def body(step, stopping_sum, running_recon, running_loss, running_digits,
                 st_ta):

            with tf.variable_scope("scale"):
                # sampling scale
                scale = tf.nn.sigmoid(
                    self._sample_from_mvn(
                        self.scale_prior_mean,
                        tf.exp(self.scale_prior_log_variance),
                        shape=[batch_size, 1],
                    ))

                # scale_value = np.mean(self.constrains_area_minmax)
                # scale_value = scale_value / self.canvas_size
                # scale = tf.ones_like(scale) * scale_value
                s = scale[:, 0]
            ...
            with tf.variable_scope("canvas"):
                ...
                # adding reconstructed window scaled
                # by z_pres to the running canvas
                condt = tf.less(stopping_sum, self.stopping_threshold)
                x = tf.reshape(window_recon, [-1, self.canvas_size * self.canvas_size])
                y = tf.zeros_like(running_recon)
            ...
            return (step + 1, stopping_sum, running_recon, running_loss, running_digits,
                    st_ta)

        with tf.variable_scope("rnn"):
            # RNN while_loop with variable number of steps for each batch item
            _, _, reconstruction, run_loss, rec_num_digits, rec_st_back = tf.while_loop(
                cond,
                body,
                [
                    tf.constant(0),  # RNN time step, initially zero
                    ...
                    tf.zeros([batch_size]),  # running value of the loss function
                    tf.zeros([batch_size],
                             dtype=tf.int32),  # running inferred number of digits
                    tf.TensorArray(dtype=tf.float32, size=0,
                                   dynamic_size=True),  # backward ST matrices
            ...
            rec_st_back = tf.transpose(
                rec_st_back.stack(), (1, 0, 2, 3), name="rec_st_back")

            bbox_reconstruction = self._visualize_generations(
                reconstruction,
                rec_st_back,
                rec_num_digits,
                zoom=2,
        ...
        return tf.reshape(
            reconstruction,
            [self.generation_batch_size, self.canvas_size, self.canvas_size, 1
            ]), bbox_reconstruction
