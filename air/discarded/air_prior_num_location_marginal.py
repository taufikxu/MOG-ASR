...
NUM_MARGINAL_COEFFICIENT = 100
SCALE_BIAS_INITIALIZER = -5
SCALE_FACTOR = 0.8
...
class AIRModel:
    ...
    def _create_model(self):
        ...
        def body(
            ...
            with tf.variable_scope("inf_scale"):
                ...
                if self.reuse_shift_scale_network is False:
                    ...
                    inf_s = inf_scale[:, 0] * SCALE_FACTOR
                else:
                    inf_s = None
            ...
            with tf.variable_scope("gen_scale"):
                ...
                gen_s = gen_scale[:, 0] * SCALE_FACTOR
            ...
            if self.constrains_x_y is not None:
                ...
                running_loss += self.constrains_x_y(inf_x / canvas_size,
                                                    inf_y / canvas_size)
                ...
                theta_recon = tf.stack(
                    [
                        tf.concat(
                            [
                                tf.stack(
                                    [1.0 / gen_s, tf.zeros_like(gen_s)], axis=1),
            ...
            with tf.variable_scope("loss/pr_num"):
                ...
                if self.constrains_num_gamma > 1e-4:
                    running_loss += tf.where(
                        ...
                        terminal_loss * self.constrains_num_gamma * 0.)
        ...
        with tf.variable_scope("loss/pr_num_margin"):
            objective = [1.] * self.max_steps
            for _c in self.constrains_num_list:
                objective[_c] = 1. - 1. / len(self.constrains_num_list)
            for _c in range(max(self.constrains_num_list), self.max_steps):
                objective[_c] = 0.
            ...
            self.constrains_num_marginal_loss = tf.reduce_sum(
                num_marginal_loss * NUM_MARGINAL_COEFFICIENT *
                self.constrains_num_gamma)

        # adding reconstruction loss
        loss = loss + self.reconstruction_loss
        ...
        with tf.variable_scope("summaries"):
            ...
            self.loss = tf.reduce_mean(loss) + self.constrains_num_marginal_loss
    ...
    def _create_generation(self):
        ...
        # body of tf.while_loop
        def body(step, stopping_sum, gen_prev_state, gen_prev_output, running_recon, running_digits):
