...
from air.air_prior_correlated_num_location import AIRModel
...
parser.add_argument(
    "-gl", "-gamma_location", "--gamma_location", type=float, default=1.0
)
...
parser.add_argument("-ds", "--dig_size", type=str, default="fixed")
parser.add_argument(
    "-NUM_REGULARIZER_TARGET_COEFFICIENT",
    "--NUM_REGULARIZER_TARGET_COEFFICIENT",
    type=float,
    default=1,
    help="mutiply a coefficient to these targeted number of digits, no matter hard or soft",
)
parser.add_argument(
    "-SCALE_BIAS_INITIALIZER", "--SCALE_BIAS_INITIALIZER", type=float, default=-5
)
parser.add_argument("-USE_HARD_LOSS", "--USE_HARD_LOSS", type=str, default="true")
args = parser.parse_args()

args.USE_HARD_LOSS = args.USE_HARD_LOSS in ["true", "t", "1"]
...
if args.dig_size in ["fix", "fixed"]:
    args.dig_size = "fixed"
else:
    args.dig_size = "varied"
...
def constrains_x_y(x, y, gamma_location=args.gamma_location):
    if args.dig_location == "right_half":
