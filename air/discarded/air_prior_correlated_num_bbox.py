...
NUM_REGULARIZER_TARGET_COEFFICIENT = 0.1
SCALE_BIAS_INITIALIZER = -5
USE_HARD_LOSS = False

# For regularization bounding box
USE_LOG_SCALE = False
BBOX_DISTANCE_MAX = 200
BBOX_SIZE_MAX = 400
    ...
    def _create_model(self):
        ...
        def body(
            ...
            scales_ta = scales_ta.write(
                scales_ta.size(), tf.concat([inf_scale * self.canvas_size], axis=-1)
            )
            shifts_ta = shifts_ta.write(
                shifts_ta.size(), tf.concat([inf_shift * self.canvas_size], axis=-1)
            )
            ...
            with tf.variable_scope("loss/pr_num"):
                ...
                if self.constrains_num_gamma > 1e-4:
                    running_loss += tf.where(
                        ...
                        * NUM_REGULARIZER_TARGET_COEFFICIENT,
                        0.0,
                    )
        ...
        with tf.variable_scope("loss/pr_bbox"):
            loop_length = tf.shape(self.z_pres_probs)[-1]
            scales1 = tf.expand_dims(self.rec_scales, axis=1)
            scales2 = tf.expand_dims(self.rec_scales, axis=2)
            shifts1 = tf.expand_dims(self.rec_shifts, axis=1)
            shifts2 = tf.expand_dims(self.rec_shifts, axis=2)
            ...
            if USE_LOG_SCALE is True:
                ...
                log_func = lambda x: x
            scale_plus = (
                tf.reduce_sum(tf.square(scales1) + tf.square(scales2), -1) + 1e-8
            )
            shifts_minus = tf.reduce_sum(tf.square(shifts1 - shifts2), -1) + 1e-8

            # ratio = tf.minimum(shifts_minus / scale_plus, tf.ones_like(shifts_minus))
            distance = tf.minimum(
                shifts_minus, tf.ones_like(shifts_minus) * BBOX_DISTANCE_MAX
            )
            # ratio = shifts_minus / scale_plus
            pr_bbox_dist_loss = -1 * log_func(distance) + BBOX_DISTANCE_MAX
            self.pr_bbox_dist_loss = (
                tf.reduce_sum(tf.reduce_sum(pr_bbox_dist_loss, axis=-1), axis=-1)
                * self.constrains_bbox_gamma
            )

            pr_bbox_size_loss = tf.reduce_mean(
                tf.reduce_sum(tf.square(self.rec_scales), axis=-1), axis=-1
            )
            pr_bbox_size_loss = (
                tf.maximum(pr_bbox_size_loss, BBOX_SIZE_MAX) - BBOX_SIZE_MAX
            )
            self.pr_bbox_size_loss = pr_bbox_size_loss * self.constrains_bbox_gamma

        # adding reconstruction loss
        loss += (
            self.reconstruction_loss + self.pr_bbox_dist_loss + self.pr_bbox_size_loss
        )
        ...
        with tf.variable_scope("summaries"):
            ...
            self._summarize_by_digit_count(
                self.pr_bbox_dist_loss, self.rec_num_digits, "pr_bbox_dist_loss"
            )
            self._summarize_by_digit_count(
                self.pr_bbox_size_loss, self.rec_num_digits, "pr_bbox_size_loss"
            )
