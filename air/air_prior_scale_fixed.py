...
class AIRModel:
    ...
    def _create_model(self):
                ...
                if self.constrains_num_gamma > 1e-4:
                    terminal_loss = tf.reduce_mean(
                        tf.nn.sigmoid_cross_entropy_with_logits(
                            labels=tf.zeros_like(z_pres_log_odds),
        ...
        with tf.variable_scope("air_model"):
            ...
            _, _, _, _, _, _, _, reconstruction, loss, self.rec_num_digits, scales, shifts, z_pres_probs, z_pres_kls, scale_kls, shift_kls, vae_kls, st_backward, windows, latents = tf.while_loop(
                cond,
                body,
                [
        ...
        with tf.variable_scope("loss/pr_bbox"):
            x1 = self.rec_shifts[:, :, 0:1] * 50
            y1 = self.rec_shifts[:, :, 1:2] * 50
            x2 = x1 + self.rec_scales * 50
            y2 = y1 + self.rec_scales * 50

            x1 = tf.maximum(x1, tf.transpose(x1, [0, 2, 1]))
            x2 = tf.minimum(x2, tf.transpose(x2, [0, 2, 1]))
            y1 = tf.maximum(y1, tf.transpose(y1, [0, 2, 1]))
            y2 = tf.minimum(y2, tf.transpose(y2, [0, 2, 1]))

            interArea = tf.maximum(x2 - x1, tf.zeros_like(x2)) * tf.maximum(
                y2 - y1, tf.zeros_like(y2))
            interArea = interArea * (
                tf.ones_like(interArea) - tf.eye(tf.shape(interArea)[-1]))
            self.interArea = interArea
            interArea = tf.reduce_mean(tf.layers.flatten(interArea), -1)
            self.num_summaries.append(
                tf.summary.scalar("pr_bbox", tf.reduce_mean(interArea)))
            if self.constrains_bbox_gamma > 1e-8:
                loss += interArea
